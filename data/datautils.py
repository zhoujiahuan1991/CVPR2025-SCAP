...
import numpy as np
from datasets import build_cd_dataset 
...
def build_test_data_loader(dataset_name, root_path, preprocess, batch_size, domain_id=None):
    if dataset_name == 'I':
        ...
        dataset = build_cd_dataset(dataset_name, root_path)
        test_loader = build_data_loader(data_source=dataset.test, batch_size=batch_size, is_train=False, tfm=preprocess, shuffle=True)
    elif dataset_name in ['D']:
        dataset, classnames, template = build_imagenet_d(root_path, domain_id, preprocess)
...
def build_test_dataset(dataset_name, root_path, preprocess, domain_id=None):
    if dataset_name == 'I':
        dataset = ImageNet(root_path, preprocess)
        ...
        test_dataset, classnames, template = build_imagenet_d(root_path, domain_id, preprocess)
            
        return test_dataset, classnames, template
    else:
        raise "Dataset is not from the chosen list"
    
    return test_dataset, dataset.classnames, dataset.template