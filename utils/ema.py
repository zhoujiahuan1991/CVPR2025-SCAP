...
class Text_EMA():
    ...
    # 带自适应权重更新一个类别的文本prompt    
    def step_one_weight(self, name, param, pred_class, w, h=37):
        assert name in self.shadow
        # print("weight:")
        # print(weight)
        # input()
        ...
        # print(new_decay)
        if self.args.ensemble and self.args.learn_all:
            new_average = self.shadow[name][:,pred_class,:,:] + w * param
            self.shadow[name][:,pred_class,:,:] = new_average.clone()
        else:
            new_average = self.shadow[name][pred_class,:,:] + w * param
            self.shadow[name][pred_class,:,:] = new_average.clone()
...
class Image_EMA():
    ...
    def apply_shadow_one(self, name, param, pred_class, w=0.5):
        # temp = []
        # print('dart retention')
        # for i in range(200):
        #     temp.append(self.shadow['image_prompts'][i].reshape(2*768).unsqueeze(0))
